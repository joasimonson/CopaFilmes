...
public class XunitTestCollectionRunnerDependencyFixture : XunitTestCollectionRunner
{
	...
	protected override void CreateCollectionFixture(Type fixtureType)
	{
		...
		else
		{
			...
			object[] ctorArgs = ctor.GetParameters().Select(p =>
			{
				if (!CollectionFixtureMappings.TryGetValue(p.ParameterType, out object arg))
				{
			...
			{
				string parameters = string.Join(", ", missingParameters.Select(p => $"{p.ParameterType.Name} {p.Name}"));
